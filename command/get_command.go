...
func init() {
	...
	getCmd.Flags().BoolVar(&getConsistentFlag, "consistent", false, "send request to the leader, thereby guranteeing that any earlier writes will be seen by the read")
...
func getCommandFunc(cmd *cobra.Command, args []string, client *etcd.Client) (*etcd.Response, error) {
	...
	return client.Get(key, getSortFlag, false)
}
